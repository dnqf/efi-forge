...
function detectChipset(boardModel: string): string {
  ...
  return [...coffeeAutoChipsets, ...cometAutoChipsets, "B450", "Z370", "H470", "H410"]
...
export function createBuildPlan(
  ...
  if (supportedGpuIds.size > 0) components.add("WhateverGreen.kext");
  if (hardware.audio.some((device) => device.vendorId.toUpperCase() === "10EC")) {
    components.add("AppleALC.kext");
  }

  if (hardware.network.some((device) => device.name.toUpperCase().includes("I219"))) {
    components.add("IntelMausi.kext");
  }
  ...
  if (
    hardware.network.some(
      (device) => device.vendorId.toUpperCase() === "10EC" && device.deviceId.toUpperCase() === "8125",
    )
  ) {
    components.add("LucyRTL8125Ethernet.kext");
  }

  if (
    hardware.network.some(
      (device) =>
        device.vendorId.toUpperCase() === "10EC" &&
        ["8168", "8161"].includes(device.deviceId.toUpperCase()),
    )
  ) {
    components.add("RealtekRTL8111.kext");
  }
  ...
  if (isAmdZen) {
    ...
    notes.push(
      ...
      "B450 不加入仅供 B550/A520 使用的 SSDT-CPUR，也不加入 Intel SSDT-PLUG/AWAC。",
    );
  }
  ...
  const igpuPlatformId = intelIgpu
    ? supportedExternalGpu
      ...
      : "07009B3E"
    : undefined;
  if (intelIgpu && blockedExternalGpu && !supportedExternalGpu) {
    bootArgs.push("-wegnoegpu");
    notes.push("检测到可用 UHD 630 与明确不受支持的独显，将默认禁用独显并由核显输出。 ");
  }
  const nativeNaviIds = new Set(["731F", "7312", "7340", "7341", "73BF", "73A3", "73FF", "73E3"]);
  if (hardware.gpus.some((device) => nativeNaviIds.has(device.deviceId.toUpperCase()))) {
    bootArgs.push("agdpmod=pikera");
  }

  const isAmdB450 = isAmdZen && chipset === "B450";
  ...
  const autoConfigSupported =
    ...
    (isAmdB450 || supportedIntelChipset);
  ...
  const smbiosModel = isAmdZen
    ...
    : isCoffee
      ...
      : isComet && hardware.cpu.cores >= 10
        ? "iMac20,2"
        : "iMac20,1";
  const setupVirtualMap = isAmdZen ? (preferences.amdSetupVirtualMap ?? true) : undefined;
  if (isAmdZen) {
    notes.push(
      setupVirtualMap
        ? "SetupVirtualMap 默认开启；若新版 B450 BIOS 在早期启动失败，可回到组装页选择关闭后重新生成。"
        : "已按用户选择关闭 SetupVirtualMap；此选项主要用于部分 2020 年末及之后的 B450/X470 BIOS。",
    );
  }
  notes.push(
