...
确认 `package.json`、`src-tauri/Cargo.toml`、`src-tauri/tauri.conf.json` 和界面版本一致。运行 CI 中的全部前端、目录导入器和 Rust 检查；涉及组件锁时还要执行四项 `--ignored` 真实下载测试。

## 2. 构建候选

在 GitHub Actions 手动运行 `Release candidate`。工作流从干净 checkout 构建 Windows x64 NSIS，生成 `SHA256SUMS.txt`，并上传保留 14 天的 Actions Artifact。
